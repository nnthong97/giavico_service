...
@RestControllerAdvice(basePackages = "com.giavico.rnd")
public class RndGlobalExceptionHandler {
    ...
    ResponseEntity<ErrorResponse> notFound(DocumentNotFoundException error, HttpServletRequest request) {
        return response(HttpStatus.NOT_FOUND, error.getMessage(), request, null);
    }

    @ExceptionHandler({InvalidWorkflowException.class, IllegalArgumentException.class})
    ResponseEntity<ErrorResponse> badRequest(RuntimeException error, HttpServletRequest request) {
        return response(HttpStatus.BAD_REQUEST, error.getMessage(), request, null);
    }

    @ExceptionHandler(PdfExportException.class)
    ResponseEntity<ErrorResponse> unprocessable(PdfExportException error, HttpServletRequest request) {
        return response(HttpStatus.UNPROCESSABLE_ENTITY, error.getMessage(), request, null);
    }
